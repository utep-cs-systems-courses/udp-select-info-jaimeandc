...
try:
    ...
    while args:
        ...
        if sw == "--clientPort":
            ...
            byteRate = float(args[0]);
            del args[0]
        elif sw == "--propLat":
            propLat = float(args[0]);
            del args[0]
        elif sw == "--pDelay":
            print("pdelay!", args[0])
            pDelay = float(args[0]);
            ...
            delayMin = float(args[0]);
            del args[0]
            if delayMin > delayMax:
                delayMax = delayMin
        elif sw == "--delayMax":
            delayMax = float(args[0]);
            del args[0]
        elif sw == "--qCap":
            qCap = int(args[0]);
            del args[0]
...
for inSock, outAddr in (
(toClientSocket, serverAddr), (toServerSocket, ("", None))):  # client's addr will be set when msg rec'd
    # send to other sock!
    transmissionSims[inSock] = TransmissionSim(otherSocket[inSock],
                                               outAddr, byteRate, propLat, pDelay, delayMin, delayMax, qCap, pDrop,
                                               pDup)
